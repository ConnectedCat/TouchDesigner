...
void main()
{
	vec4 position = texelFetch(sTD2DInputs[0], ivec2(gl_GlobalInvocationID.xy), 0);
	vec4 velocity = texelFetch(sTD2DInputs[1], ivec2(gl_GlobalInvocationID.xy), 0);
	vec4 initState = texelFetch(sTD2DInputs[2], ivec2(gl_GlobalInvocationID.xy), 0);

	vec3 pos = position.rgb;
	vec3 vel = velocity.rgb;
	vec3 ini = initState.rgb;
